...
class CubeGame:
  ...
  def min_checks( self ):
    for i, round in enumerate( self.rounds ):
      print( f"Game {self.game} >> Round {i} >> rgb( {round.reds}, {round.greens}, {round.blues} )" )
      self.min_reds   = round.reds   if round.reds   > self.min_reds   else self.min_reds
      self.min_greens = round.greens if round.greens > self.min_greens else self.min_greens
      self.min_blues  = round.blues  if round.blues  > self.min_blues  else self.min_blues
    
  def read_game( self ):
    return self.game if self.is_legal else 0
  
  def read_powers( self ):
    return self.min_reds * self.min_greens * self.min_blues
  
  def to_string( self ):
    return f"Game {self.game} >> rgb( {self.min_reds}, {self.min_greens}, {self.min_blues} ) >> Power: {self.read_powers()}"
...
def day_2_b():
  with open( PATH_TO_DAY_TWO_A ) as file:
    ...
    print( sum( [ game.read_powers() for game in games ] ) )